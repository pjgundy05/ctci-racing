...
# =========================
# Default weights (you can tune live in the sidebar)
# =========================
DEF_WEIGHTS = {
    ...
    "class_rise": -6,
    "protected_spot": 4,           # e.g., OC-N / allowance N1X protection

    # Form cycle
    "highest_last_fig": 10,        # "Highest last race speed rating"
    "last_out_2nd": 6,             # "Finished 2nd in last race"
    "last_out_3rd": 4,
    "hot_trn_jky": 6,              # "Hot Tnr/Jky combo"
    "second_off_layoff": 8,        # "2nd off layoff" / "Second off layoff"
    "layoff_penalty": -6,          # "Has not raced for more than 2 months"
    "sharp_workout": 5,            # detect bullet-style works "B 1/xx" or "1/xx"
    ...
    # Pace model
    "pace_lone_speed": 10,
    "pace_speed_help": 5,          # soft/avg pressure helps E/EP
    "pace_closer_help": 6,         # hot pressure helps P/S
    "pace_hot_threshold": 4,       # >= this # of E/EP(>=5) = hot
}

DEFAULT_PP = 100
DEFAULT_STYLE = ("P", 3)  # if style not parsed
HORSE_SPLIT = re.compile(r"\n(?=\s*\d+\s+[A-Za-z][^\n]+?\()")  # start of each horse card
...
def analyze_single_race_text(text, weights):
    ...
    if not horses: 
        return pd.DataFrame(), {"surface": surface, "distance_type": dist_type, "furlongs": furlongs,
...
def build_tickets(df, meta, budget, risk="balanced"):
    ...
    tri_combos, tri_tickets, t_spent = [], [], 0.0
    for a in A:
        for b in B:
            if b==a: continue
            for c in C:
    ...
    summary = {"budget": budget, "win_bank": win_bank,
               "exa_bank": math.floor(spent), "tri_bank": math.floor(t_spent),
               "risk": risk, "meta": meta}
    flat = []
